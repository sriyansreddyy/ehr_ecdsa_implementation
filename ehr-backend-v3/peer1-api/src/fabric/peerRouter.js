...
const PEER_ADDR = process.env.PEER_ADDRESS || 'localhost:9051';
const TLS_CERT  = path.join(PEERS, 'peer1.hospital.example.com/tls/ca.crt');
const MSP_ID    = 'HospitalMSP';

// peer1 serves: doctor only
const PEER_MAP = {
  doctor: {
    address: PEER_ADDR, tlsCertPath: TLS_CERT,
    mspPath: path.join(USERS, 'doctor/msp'), mspId: MSP_ID,
  },
};
